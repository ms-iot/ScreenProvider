...
#define SHIFT_FOR_UPPER_5_IN_565   (11)
#define SHIFT_FOR_MIDDLE_6_IN_565  (5)
#define SHIFT_FOR_UPPER_8  (8)
#define SHIFT_FOR_MIDDLE_4  (4)
#define UPPER_4_BITS(c)   (((c) & rMaskTable[4 - 1]) >> 4)
...
#define CONVERT_32BPP_TO_16BPP(pPixel) ((UPPER_5_BITS(pPixel[0]) << SHIFT_FOR_UPPER_5_IN_565)  | \
                                        (UPPER_6_BITS(pPixel[1]) << SHIFT_FOR_MIDDLE_6_IN_565) | \
                                        (UPPER_5_BITS(pPixel[2])))

#define MSB_4(c) ((c & 0xF0) >> 4)
#define LSB_4(c) ((c & 0x0F))
#define FLIP_16_BIT(pPixel) (MSB_4(pPixel[0]) | \ LSB_4(pPixel[0]) << 4 | \ MSB_4(pPixel[1]) << 8 | \ LSB_4(pPixel[1]) << 12)

/* #define CONVERT_32BPP_TO_16BPP(pPixel) ((pPixel[2] << SHIFT_FOR_UPPER_8)  | \
                                        (UPPER_4_BITS(pPixel[1]) << SHIFT_FOR_MIDDLE_4) | \
                                        (UPPER_4_BITS(pPixel[0]))) */
        ...
        for (UINT y=0; y < NumRows; y++)
        {
            ...
            for (UINT x=0; x < NumPixels; x++)
            {
                ...
                else if (pDst->BitsPerPel == 16)
                {
					...
					*pDstPixelAs16 = ((pDstPixel[0]) << 8 | pDstPixel[1]);
                }
